...
import { getGrid, isCutCell, cellKey, PX_PER_INCH } from '@/core/grid';

const TILE_INCH = 7.87;
const TILE_PX = TILE_INCH * PX_PER_INCH;
...
describe('getGrid', () => {
  it('returns full grid with no cuts for exact fit', () => {
    const g = getGrid(exactFit, TILE_INCH);
  ...
  it('adds fractional col when width overhangs', () => {
    const g = getGrid(fracCol, TILE_INCH);
  ...
  it('adds fractional row when height overhangs', () => {
    const g = getGrid(fracRow, TILE_INCH);
  ...
  it('adds both fractional row and col', () => {
    const g = getGrid(fracBoth, TILE_INCH);
    expect(g.cols).toBe(5);
    expect(g.rows).toBe(4);
    expect(g.hasColCut).toBe(true);
    expect(g.hasRowCut).toBe(true);
  ...
  it('treats sub-epsilon remainder as no cut', () => {
    const g = getGrid({ widthIn: TILE_INCH * 4 + 0.001, heightIn: TILE_INCH * 3 }, TILE_INCH);
    expect(g.hasColCut).toBe(false);
    expect(g.cols).toBe(4);
  });
  ...
  it('respects a different tile size', () => {
    const g = getGrid({ widthIn: 12, heightIn: 8 }, 4);
    expect(g.cols).toBe(3);
    expect(g.rows).toBe(2);
    expect(g.fullCols).toBe(3);
    expect(g.fullRows).toBe(2);
    expect(g.hasColCut).toBe(false);
    expect(g.hasRowCut).toBe(false);
    // Each cell should be 4 inches wide in pixels.
    expect(g.colSizes[0]).toBeCloseTo(4 * PX_PER_INCH, 2);
  });
});
